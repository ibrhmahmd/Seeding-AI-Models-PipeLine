...
input_tags_file = "Tags/missing_tags.md"
...
with open(log_file, 'a') as log:
    log.write(f"Starting tag creation process at {time.ctime()}\n")
    log.write(f"Target API: {api_url}\n")
    log.write(f"Reading tags from: {input_tags_file}\n")
    log.write(f"Outputting IDs to: {ids_file}\n")
    ...
    log.write(f"\nFinished tag creation process at {time.ctime()}\n")
